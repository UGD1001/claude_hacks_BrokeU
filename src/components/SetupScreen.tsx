...
export default function SetupScreen({ onStart, onBack }: Props) {
  ...
  const diffLabel = () => {
    if (annualNet > 40000) return { text: 'EASY', color: 'var(--green)' }
    if (annualNet > 20000) return { text: 'MEDIUM', color: 'var(--yellow)' }
    if (annualNet > 5000)  return { text: 'HARD', color: 'var(--orange)' }
    return { text: 'BRUTAL', color: 'var(--red)' }
  }
      ...
      <div className="setup-left">
        ...
        <div className="setup-field">
          ...
          <div className="setup-options">
            {SALARY_OPTIONS.map(s => (
              <button
                key={s}
                className={`setup-opt ${salary === s ? 'active' : ''}`}
                onClick={() => setSalary(s)}
              >
                {fmt(s)}
              </button>
        ...
        <div className="setup-field">
          ...
          <div className="setup-options">
            {RENT_OPTIONS.map(r => (
              <button
                key={r}
                className={`setup-opt ${rent === r ? 'active' : ''}`}
                onClick={() => setRent(r)}
              >
                {fmtMo(r)}
              </button>
        ...
        <div className="setup-field">
          <label className="setup-label">
            MONTHLY EXPENSES — {fmtMo(expenses)}
          </label>
          <input
        ...
        <div className="setup-field">
          ...
          <div className="setup-options">
            {TUITION_OPTIONS.map(t => (
              <button
                key={t.value}
                className={`setup-opt ${tuition === t.value ? 'active' : ''}`}
                onClick={() => setTuition(t.value)}
              >
                {t.label}
              </button>
            ))}
        ...
        <div className="setup-field" style={{ marginTop: 8 }}>
          <div className="setup-fixed-note">Starting cash: $500 (fixed for all)</div>
        </div>
      ...
      <div className="setup-right">
        <div className="setup-preview">
          ...
          <div className="setup-diff-row">
            ...
            <span className="setup-diff-badge" style={{ color: diff.color, border: `1px solid ${diff.color}` }}>
              {diff.text}
            </span>
          </div>

          <div className="setup-preview-divider" style={{ marginTop: 20 }} />

          <div className="setup-preview-title" style={{ marginTop: 16 }}>COMPUTER OPPONENT</div>
          <div className="setup-preview-block">
            <div className="setup-preview-row">
              <span>Salary</span>
              <span className="pos">+{fmtMo(salary / 12)}</span>
