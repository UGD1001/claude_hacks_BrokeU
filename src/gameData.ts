...
export const YEAR_SEC = 60        // real seconds per game year
export const CAR_GOAL = 25000     // phase 1 target
...
export const COMP_RENT      = 1200   // monthly
export const COMP_EXPENSES  = 800    // monthly
...
export const LIFE_EVENTS: GameEvent[] = [
  ...
  },
  {
    ...
    choices: [
      ...
      { key: 'B', label: 'Start a side hustle to offset', expenseAddMonthly: 800, outcomeClass: 'neutral', outcome: '+$800/mo — consider a side hustle' },
    ],
  },
  {
    id: 'rent_hike',
...
export function getSideHustleAnnualIncome(s: GameState): number {
  ...
  for (const h of SIDE_HUSTLES) {
    ...
    if ('growthYears' in h && h.growthYears) {
      const multiplier = Math.min(4, 1 + yearsActive)
